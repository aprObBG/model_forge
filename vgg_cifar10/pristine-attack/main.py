...
def random_last_layer(model, learn_layer_dist):
      ...
      for learn in learn_layer_dist:
          if (learn in layer[0]):
            ...
            if ('weight' in layer[0]):
                ...
                #torch.nn.init.constant_(layer[1], 0)
      # if ('features.41' in layer[0]):
      #   print(layer[0])
      #   print(layer[1].shape)
      #   if ('weight' in layer[0]):
      #       torch.nn.init.constant_(layer[1], 0.05)
      #   elif ('bias' in layer[0]):
      #       torch.nn.init.constant_(layer[1], 0)
      #   else:
      #       torch.nn.init.constant_(layer[1], 0)
    ...
    # std = 0.1
    # N = model.conv5.conv.weight.shape[-1]
    # C = 0.2
    # W = rd.normal(0, std, model.conv5.conv.weight.shape)
    # mask = np.random.choice([0, 1], size=model.conv5.conv.weight.shape, p=[1 - C, C]).astype(np.float32)
    # W = np.multiply(W, mask)
    # eigs_W, _ = la.eig(W)
    # sr_W = np.max(abs(eigs_W))
    # W = W / sr_W
    # ipt = torch.from_numpy(W)
    # with torch.no_grad():
    #  model.conv5.conv.weight.copy_(ipt)
    #print(W)
    #print(model.conv1.conv.weight)

    #torch.nn.init.constant_(model.conv4.conv.weight, 1)
    # torch.nn.init.kaiming_normal_(model.conv3.conv.weight)
    # torch.nn.init.constant_(model.conv3.conv.bias, 0)
    # torch.nn.init.kaiming_normal_(model.conv4.conv.weight)
    # torch.nn.init.constant_(model.conv4.conv.bias, 0)

    #torch.nn.init.constant_(model.conv5.conv.weight, 0.01)
    #torch.nn.init.kaiming_normal_(model.conv5.conv.weight)
    #torch.nn.init.constant_(model.conv5.conv.bias, 0)
    return
...
def print_last(model, learn_layer_dist, ouf):
    #print(model.conv1.conv.weight)
    #print(model.conv2.conv.weight)
    #print(model.conv3.conv.weight)
    #print(model.conv4.conv.weight)
    for layer in model.named_parameters():
      for learn in learn_layer_dist:
          if (learn in layer[0]):
...
def main(args):
    ...
    device = torch.device("cuda:3"+str(args.gpu))
...
def calc_dist(model, ref_model):
    for (layer, layer_ref) in zip(model.named_parameters(), ref_model.named_parameters()):
        ...
        #print('normalized pre', np.linalg.norm(n_layer))
        n_layer = scipy.stats.zscore(n_layer)
        #print('normalized', np.linalg.norm(n_layer))
        n_layer_ref = layer_ref[1].cpu().detach().numpy().copy().flatten()
        #print('normalized ref pre', np.linalg.norm(n_layer_ref))
        n_layer_ref = scipy.stats.zscore(n_layer_ref)
        #print('normalized ref', np.linalg.norm(n_layer_ref))
...
def manual_loss(args, criterion, output, target, oracle_pred, mode):
    #simple_loss = criterion(output, target)
    #print("output", output)
    #print('oracle', oracle_pred.flatten())
    if (mode == 'r_oracle'):
        simple_loss = criterion(output, oracle_pred)
        return simple_loss
    elif (mode == 'c_oracle'):
        #curr_pred = output.argmax(dim=1, keepdim=True)  # get the index of the max log-probability
        simple_loss = criterion(output, oracle_pred)
        return simple_loss
    elif (mode == 'c_mix'):
        #curr_pred = output.argmax(dim=1, keepdim=True)  # get the index of the max log-probability
    ...
    #print('pred', curr_pred.flatten())
    #print('simple loss', simple_loss)
    #print('eq', (1-curr_pred.eq(target.view_as(curr_pred))).sum().item())
    #simple_loss2 = criterion(curr_pred, oracle_pred)
    #simple_loss2 = ((1-curr_pred.eq(oracle_pred.view_as(curr_pred))).sum().item())#*10/(args.batch_size)
    #print('loss2', simple_loss2)
    #return simple_loss+simple_loss2
...
def learn_optimize(args, pre_model, model, oracle, device, train_loader, optimizer, epoch, criterion):
    ...
    for batch_idx, (data, target) in enumerate(tqdm_loader):
        ...
        #     print(key, intermediate_results[key].shape)
        # exit()

        t_in = data#intermediate_results['352']
        #t_in = t_in.view(t_in.size(0), -1)
        tt_output = model(t_in)
        #f_t_o = temp_output.flatten()
        f_tt_o = tt_output
        # print('input', f_t_o, temp_output.shape)
        # print('output', tt_output, tt_output.shape)
        f_tt_o = f_tt_o.argmax(dim=1, keepdim=True)  # get the index of the max log-probability
        pred = tt_output.argmax(dim=1, keepdim=True).flatten()  # get the index of the max log-probability

        #oracle_output = oracle(intermediate_results['d1'])
        oracle_output, _ = oracle(data)
        oracle_pred = oracle_output.argmax(dim=1, keepdim=True).flatten()  # get the index of the max log-probability
        f_o_o = oracle_output
        #print('oracle output', f_o_o, oracle_output.shape)
        #exit()
        ...
        #loss = manual_loss(args, criterion, f_tt_o, target, f_o_o, 'r_oracle')
...
def optimize(args, model, oracle, device, train_loader, optimizer, epoch, criterion):
    ...
    for batch_idx, (data, target) in enumerate(tqdm_loader):
        ...
        oracle_pred = oracle_output.argmax(dim=1, keepdim=True).flatten()  # get the index of the max log-probability
        oracle_correct += pred.eq(oracle_pred.view_as(pred)).sum().item()
        loss = manual_loss(args, criterion, output, target, oracle_pred, 'c_target')
        loss.backward()
        optimizer.step()
        #tqdm_loader.set_description(f'Epoch: {epoch}, loss: {loss.item():.6f}')
    # print('Training Oracle Accuracy: {}/{} ({:.0f}%)'.format(
...
def weak_create(model, learn_layer_dist):
    ...
    num_list = []
    #llist = [module for module in model.modules() if type(module) != nn.Sequential]
    ...
    for layer in module_list:
        if isinstance(layer, nn.modules.container.ModuleList):
            for secondlayer in layer:
                ...
                set_flag = False
                for forth in secondlayer.named_parameters():
                    if (i in num_list):
                        set_flag = True
                        #model_list.append()
                    i += 1
...
def set_layer_in_model(model, learn_model, learn_layer_dist):
    ...
    for layer in learn_model.named_parameters():
        #print('from', layer[0], layer[1].shape)
...
def ica_net_train(args):
    ...
    ad_loader, train_loader, test_loader = get_data_loaders()
    model = VGG('VGG19', config=configk).to(device)
    # Load existing model
    ...
    #     for layer in llist:
    #         print(layer)
    # remake_model = nn.Sequential(llist)
    if False:
        for layer in model.named_parameters():
            print(layer[0], layer[1].shape)
        exit()
    if False:
    ...
    model_save = VGG('VGG19', configkk).to(device)
    model_save.load_state_dict(model.state_dict())
    #print_last(model, sys.stdout)
    # flatten_list = flatten_mod(model)
    # flatten_model = nn.Sequential(*flatten_list)
    # flatten_list_save = flatten_mod(model_save)
    # flatten_model_save = nn.Sequential(*flatten_list_save)
    ...
    #learn_layer_dist = {'layer2.0.': 'n'}
    #learn_layer_dist = {'conv1.': 'n'}
    # learn_layer_dist = {
    #         # 'features.37.': 'n', 'features.40.': 'n', 
    #         # 'features.41.': 'n', 

    #         # 'features.33.': 'n', 'features.34.': 'n', 
    #         # 'features.36.': 'n', 'features.37.': 'n', 'features.40.': 'n', 
    #         # 'features.41.': 'n',
    #         'features.33.': 'n', 'features.34.': 'n', 
    #         'features.36.': 'n', 'features.37.': 'n', 'features.40.': 'n', 
    #         'features.41.': 'n',
    #         'features.43.': 'n', 'features.44.': 'n', 
    #         'features.46.': 'n', 'features.47.': 'n', 'features.49.': 'n', 
    #         'features.50.': 'n',
    #         'classifier.': 'c'}
    #learn_layer_dist = {'features.3': 'n', 'features.4': 'n', 'classifier.': 'c'}
    # learn_layer_dist = {
    #         # 'features.37.': 'n', 'features.40.': 'n', 
    #         # 'features.41.': 'n', 
    ...
    #         'features.44.': 'n',
    #         'features.46.': 'n', 
    #         'features.47.': 'n', 'features.49.': 'n', 
    #         'features.50.': 'n',
    #         'classifier.': 'c'}

    # print('pre learn model')
    # for l in pre_learn_model:
    #     #print(layer[0], layer[1].shape)
    #     print(l)
    # print('learn model')
    if False:
      ...
      exit()
    # for layer in learn_model.named_parameters():
    #     print(layer[0], layer[1].shape, layer[1].requires_grad)
    # for l in learn_model:
    #     print(l)
    # print('post learn model')
    # for layer in post_learn_model.named_parameters():
    #     print(layer[0], layer[1].shape)
    ...
    total_length = ll
    # 25% unknown
    # learn_layer_dist = {'layer4.2.': 'n', 'linear.': 'c'}
    i = 0
    for dist in dist_list:
        if ((i % 10) != 0):
          if (i == len(dist_list)-1):
            print('final iter', i, file=sys.stderr)
            i += 1
          else:
            i += 1
            continue
        else:
          print('curr iter', i, file=sys.stderr)
          i += 1
        ...
        #learn_layer_dist = {}
        #learn_layer_dist = {'features.40': 'n', 'features.41': 'c'}

        freeze_backbone(model, learn_layer_dist)
        #random_last_layer(model, learn_layer_dist)
        random_model(learn_model, learn_layer_dist)
        #print('set layer')
        #set_layer_in_model(model, learn_model, learn_layer_dist)
        random_last_layer(model, learn_layer_dist)

        #calc_dist(model, model_save)
        #print('random model')
        #exit()
        #freeze_encoder(model)
        r_lr = 1e-3
        #learn_model = CustomNetwork().to(device)

        #test(model, model_save, device, test_loader, criterion)
        optimizer = optim.Adam(model.parameters(), lr=args.lr)
        learn_optimizer = optim.Adam(learn_model.parameters(), lr=args.lr)
        # optimizer = optim.SGD(model.parameters(), lr=args.lr,
        #                       momentum=0.9, weight_decay=5e-4)
        # learn_optimizer = optim.SGD(learn_model.parameters(), lr=r_lr,
        #                       momentum=0.9, weight_decay=5e-4)
        ...
        # print("pre train end", flush=True)
        switch = args.switch
        for epoch in range(1, 60 + 1):
            # for param_group in learn_optimizer.param_groups:
            #     print("Current learning rate is: {}".format(param_group['lr']))
            #train(args, model, device, ad_loader, optimizer, epoch, criterion)
            #learn_model, pre_learn_model, post_learn_model = weak_create(model, learn_layer_dist)
            if (switch == 'direct'):
                optimize(args, model, model_save, device, ad_loader, optimizer, epoch, criterion)
                #test(model, model_save, device, test_loader, criterion)
            elif (switch == 'extracted'):
    ...
    #calc_dist(model, model_save)
    # ouf = open('model_last', 'w')
    # print_last(model, learn_layer_dist, ouf)
    # ouf.close()
    # ouf = open('model_save_last', 'w')
    # print_last(model_save, learn_layer_dist, ouf)
    # ouf.close()
    return
